...
public class EligibilityScoreService {
    ...
    public int calculateCategoryScore(Application application) {
        ...
        String beneficiaryCategory = application.getBeneficiary().getCategory();
        for (SchemeCategory schemeCategory : schemeCategories) {
            if (schemeCategory.getCategory().equalsIgnoreCase(beneficiaryCategory)) {
                return 40;
            }
        }
        return 0;
    ...
    public int calculateTotalScore(Application application) {
        return calculateIncomeScore(application) + calculateCategoryScore(application) + calculateDocumentScore(application);
    }

    public void evaluateApplication(Application application) {
        int totalScore = calculateTotalScore(application);
        ...
        if (totalScore >= application.getScheme().getMinimumScore()) {
            ...
            application.setRejectionReason("Eligibility score below minimum threshold.");
        }
    }
    ...
    public String getEvaluationSummary(Application application) {
        ...
        return "Income Score : " + income +
                ...
                "\nTotal Score : " + total +
                "\nEligibility : " +
                (total >= application.getScheme().getMinimumScore() ? "ELIGIBLE" : "INELIGIBLE");
    }
}
